    ...
    */
    ...
    Deferred: function () {
      ...
      var promise = {
        ...
        // 将promise对象合并到传入进来的对象中
        promise: function (obj) {
          return typeof obj === 'object' ? jQuery.extend(obj, promise) : promise
        }
      }
      // 数据源的集合
      // done和fail都是对应的callback.add()；它们都用来添加订阅者
      // resolve和reject；它们都是发布者；发布消息，通知所有订阅者。
      var tuples = [
        /* 数组最后一个代表的是状态，倒数第二个值是Callbacks
        数组第一个值，代表的是给deferred扩展的方法名，对应的是一个订阅者；
        数组第二个参数，代表的是 新增观察者的方法
        */
      ...
      jQuery.each(tuples, function (i, tuple) {
        ...
        // promise.resolve || reject || notify 设置为fire方法
        deferred[tuple[0]] = callback.fire
        // promise.resolveWith || rejectWith || notifyWith 设置为fireWith方法
        deferred[tuple[0] + 'With'] = callback.fireWith
        // 给promise.done || fail || progress 设置为add方法
    ...
    /**
     * 
     * @param { deferred对象 } async 
     */
    when: function (async) {
      return async.promise() // 这里返回的就是 promise对象
    }
  })
  /**
   * 
